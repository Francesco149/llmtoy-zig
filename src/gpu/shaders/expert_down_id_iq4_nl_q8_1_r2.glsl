#version 450
// Two-row workgroup variant of expert_down_id_iq4_nl_q8_1.glsl.
//
// Each 64-thread workgroup contains two 32-lane row groups. This preserves the
// original one-row dot-product math while cutting the workgroup count in half.
...
void main() {
    ...
    const uint row = gl_WorkGroupID.x * 2u + row_in_group;
    ...
    for (uint sub = lane; sub < n_sub; sub += 32u) {
        ...
        [[unroll]] for (uint j = 0u; j < 16u; ++j) {
            ...
            const uint act_lane = j & 3u;
            const int a0 = act_byte(acts[x4_idx].qs[act_off + word_idx], act_lane);
            const int a1 = act_byte(acts[x4_idx].qs[act_off + 4u + word_idx], act_lane);
