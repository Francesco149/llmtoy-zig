#version 450
// Fused add + RMS normalization (single-row):
// y[i] = (a[i] + b[i]) * rms_inv * w[i]
// where rms_inv = 1 / sqrt(sum((a[i] + b[i])^2) / n + eps).
...
layout(push_constant) uniform PC {
    uint  n;
    float eps;
    uint  flags; // bit 0 = use 1+w, bit 1 = CPU-order scalar sum
} pc;
