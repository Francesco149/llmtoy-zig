#version 450
// Batched selected-expert gate+up for Gemma4 MoE.
//
// Weight layout is the raw GGUF expert tensor:
//   expert e: gate rows [0, rows), then up rows [0, rows)
// Output layout is slot-major:
//   out[slot * rows + row] = gelu(gate(e,row) dot acts) * (up(e,row) dot acts)
...
void q3_dot_pair(uint gate_block_base, uint up_block_base, uint tid, uint n_sub, out float gate_acc, out float up_acc) {
    gate_acc = 0.0;
    up_acc = 0.0;
...
void main() {
    ...
    float gate_acc;
    float up_acc;
    q3_dot_pair(gate_base, up_base, tid, n_sub, gate_acc, up_acc);
