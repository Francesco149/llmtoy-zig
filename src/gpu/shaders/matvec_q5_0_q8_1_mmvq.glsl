#version 450
// Experimental Q5_0 x Q8_1 MMVQ-style matvec.
//
// Mirrors llama.cpp's DATA_A_Q5_0 legacy-quant path in mul_mat_vecq.comp:
// K_PER_ITER=8, packed16 quant reads, Q8_1 block cache, and
// subgroup-plus-shared-memory reduction. Bench-only until it wins.
...
layout(std430, set = 0, binding = 0) readonly  buffer WP16 { block_q5_0_packed16 weights_p16[]; };
...
i32vec2 repack(uint ib, uint iqs) {
    ...
    const uint32_t qh = (uint32_t(weights_p16[ib].qh[1]) << 16u)
                      | uint32_t(weights_p16[ib].qh[0]);
...
float mmvq_dot_product(uint ib_a, uint iqs) {
    ...
    return float(weights[ib_a].d) * (float(q_sum) * cache_b_ds.x - 4.0 * cache_b_ds.y);
}
